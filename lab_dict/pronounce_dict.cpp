...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string first = word1;
    string second = word2;
    for (char & cfirst: first) {
      cfirst = toupper(cfirst);
    }
    for (char & csecond: second) {
      csecond = toupper(csecond);
    }
    map<std::string, std::vector<std::string>>::const_iterator iter1 = dict.find(first);
    map<std::string, std::vector<std::string>>::const_iterator iter2 = dict.find(second);
