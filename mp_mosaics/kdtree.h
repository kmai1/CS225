...
class KDTree
{
  ...
  private:
     ...
     */
     unsigned int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIndex, size_t dim);
     Point<Dim> quickSort(vector<Point<Dim>>& newPoints, int left, int right, int k, size_t dim);
     void createKDT(vector<Point<Dim>>& newPoints, int left, int right, size_t dim, KDTreeNode*& root);
     void remove(KDTreeNode* root);
     void copy(KDTreeNode* curr, KDTreeNode* other);
     Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode* subroot, size_t dim) const;
