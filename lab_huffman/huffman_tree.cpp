  ...
  //std::cout << "removesmall" << std::endl;
     ...
     */
     ...
     if (mergeQueue.size() == 0) {
      ...
      TreeNode* smallSingle = singleQueue.front();
      TreeNode* smallMerge = mergeQueue.front();
      if (smallSingle->freq.getFrequency() <= smallMerge->freq.getFrequency()) {
        singleQueue.pop();
        smallest = smallSingle;
      } else {
        mergeQueue.pop();
        smallest = smallMerge;
      }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
  //std::cout << "build tree" << std::endl;
     ...
     */
     for (unsigned long k = 0; k < frequencies.size(); k++) {
       TreeNode* node = new TreeNode(frequencies[k]);
       singleQueue.push(node);
     }
     while (singleQueue.size() + mergeQueue.size() != 1) {
       TreeNode* firstSmall = removeSmallest(singleQueue, mergeQueue);
       TreeNode* secondSmall = removeSmallest(singleQueue, mergeQueue);

       TreeNode* internalNode = new TreeNode(firstSmall->freq.getFrequency() + secondSmall->freq.getFrequency());
       internalNode->left = firstSmall;
       internalNode ->right = secondSmall;
      ...
      //std::cout << "decode here" << std::endl;
         ...
         */
         ...
         if (bfile.getNextBit()) {
           ...
           //std::cout << "decode2" << std::endl;
           current = current->left;
         }
         //leaf node check
         //std::cout << "decode3" << std::endl;
         if (current->left == NULL && current->right == NULL) {
           //std::cout << "decode4" << std::endl;
           ss << current->freq.getCharacter();
           //std::cout << "decode5" << std::endl;
           //transverse to root node again
           current = root_;
           //std::cout << "decode6" << std::endl;
         }
         //std::cout << "decode6" << std::endl;
    }
}
  ...
  //std::cout << "read tree" << std::endl;
     ...
     */
     ...
     if (bfile.getNextBit()) {
       ...
       Frequency temp(0);
       ans = new TreeNode(temp);
       ans->left = readTree(bfile);
       ans->right = readTree(bfile);
     }
    return ans;
}
