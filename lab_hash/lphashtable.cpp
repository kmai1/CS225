...
int LPHashTable<K, V>::findIndex(const K& key) const
{
     ...
     */
     ...
     unsigned int originalSpot = curr;
     //should_probe see if we need to look forward
     while (should_probe[curr]) {
       if (table[curr] != NULL && table[curr]->first == key) {
         return curr;
       }
       curr = ((curr + 1) % size);
       //needed for inifnite loop
       if (curr == originalSpot) {
         break;
       }
     }
    return -1;
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     */
     ...
     for (size_t k = 0; k < newSize; k++) {
       newTable[k] = NULL;
       should_probe[k] = false;
     }
     //copy over
     for (size_t k = 0; k < size; k++) {
       if (table[k] != NULL) {
         size_t index = hashes::hash(table[k]->first, newSize);
         while(should_probe[index]) {
